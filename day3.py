...
def get_points(instr: list) -> set:
    ...
    for i in instr:
        if i[0] == "R":
            a = current_coord[0] + 1
            b = a + int(i[1:])
            [final.add((x, current_coord[1])) for x in range(a, b)]
            current_coord = (b - 1, current_coord[1])
        elif i[0] == "L":
            a = current_coord[0] - 1
            b = a - int(i[1:])
            [final.add((x, current_coord[1])) for x in range(a, b, -1)]
            current_coord = (b + 1, current_coord[1])
        elif i[0] == "U":
            a = current_coord[1] + 1
            b = a + int(i[1:])
            [final.add((current_coord[0], y)) for y in range(a, b)]
            current_coord = (current_coord[0], b - 1)
        elif i[0] == "D":
            a = current_coord[1] - 1
            b = a - int(i[1:])
            [final.add((current_coord[0], y)) for y in range(a, b, -1)]
            current_coord = (current_coord[0], b + 1)
        # print(current_coord)
    return final

def part1(one: list, two: list) -> int:
